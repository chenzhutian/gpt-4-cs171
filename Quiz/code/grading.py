...
def grade_multiple_answers_question_old(student_answer, root):
    # Extract metadata
    points_possible = float(root.find(".//qtimetadatafield[fieldlabel='points_possible']/fieldentry").text)
    original_answer_ids = set(
        root.find(".//qtimetadatafield[fieldlabel='original_answer_ids']/fieldentry").text.split(','))
    ...
    for condition in resprocessing.findall('respcondition'):
        ...
        if conditionvar is not None and setvar is not None:
            ...
            and_condition = conditionvar.find('and')
            not_conditions = and_condition.findall('not/varequal')

            all_not_conditions_passed = all(
                [str(answer_id.text) not in student_answer_ids for answer_id in not_conditions])

            if all_not_conditions_passed:
                correct_answers = set([str(e.text) for e in and_condition.findall('varequal')])
                if correct_answers.issubset(student_answer_ids):
                    return value / maxvalue * points_possible
